...
export default async function MostListening({ mostListening }) {
    const topthree = await mostListening();
    return (
        <>
            {(!topthree) ?
                <span className="text-senary-color font-extrabold flex items-center gap-2"><GiNightSleep className="w-6 h-6" />Ninguém ouviu nada ainda</span>
                :
                (
                    topthree?.map(({ Song, Artist }, i) => {
                        return (
                            <Link key={Artist.id} href={`/artists/${encodeURL(Artist.id)}-${encodeURL(Song.id)}`} >
