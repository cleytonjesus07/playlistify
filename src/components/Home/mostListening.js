                    ...
                    topthree?.map(({ Song, Artist }, i) => {
                        return (
                            <Link key={Artist.id} href={`/${lang}/artists/${encodeURL(Artist.id)}-${encodeURL(Song.id)}`} >
                                <Container className={"flex items-center justify-center relative  hover:scale-100 scale-95 cursor-pointer transition-all ease-in-out"}>

                                    <div className=" relative overflow-hidden bg-senary-color w-20 h-20 rounded-tl-xl rounded-bl-xl">
